...
contains
  ...
  subroutine mn_phys_fill_nest_halos_from_parent(Atm, IPD_Control, IPD_Data, n, child_grid_num, is_fine_pe, nest_domain, nest_level, nz)
    ...
    if (move_physics) then
      ...
      call fill_nest_halos_from_parent("usfco", mn_phys%usfco, interp_type, Atm(child_grid_idx)%neststruct%wt_h, &
          Atm(child_grid_idx)%neststruct%ind_h, &
          x_refine, y_refine, &
          is_fine_pe, nest_domain, position, nest_level)
      call fill_nest_halos_from_parent("vsfco", mn_phys%vsfco, interp_type, Atm(child_grid_idx)%neststruct%wt_h, &
          Atm(child_grid_idx)%neststruct%ind_h, &
          x_refine, y_refine, &
          is_fine_pe, nest_domain, position, nest_level)
