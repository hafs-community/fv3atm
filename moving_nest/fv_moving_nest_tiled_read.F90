...
!    if (this_pe .eq. 1199) print '("[INFO] WDR FX grid_offsets npe=",I0," nx=",I0," ny=",I0," half_nx=",I0," half_ny=",I0," ioffset=",I0," joffset=",I0)',this_pe, nx, ny, half_nx, half_ny, ioffset, joffset
  ...
  subroutine check_update_static_tile_data(fp_nx, fp_ny, nest_nx, nest_ny, ioffset, joffset, refine, a_step, st, surface_dir, pelist, parent_tile, month, use_timers, id_movnest_readstatic, do_read)
    ...
    if (do_read) then
      ...
      call mn_static_read_tiled_hires_r4(new_tile_ioffset, new_tile_joffset, tile_nx, tile_ny, refine, pelist, surface_dir, "substrate_temperature", "substrate_temperature", st%deep_soil_temp_grid, num_bytes, parent_tile)
      total_bytes = total_bytes + num_bytes
      ! set any -999s to +4C
      call mn_replace_low_values(st%deep_soil_temp_grid, -100.0, 277.0)
      ...
      call mn_replace_low_values(st%soil_type_grid, -100.0, 0.0)
      ...
      call mn_static_read_tiled_hires_r4(new_tile_ioffset, new_tile_joffset, tile_nx, tile_ny, refine, pelist, surface_dir, "vegetation_type", "vegetation_type", st%veg_type_grid, num_bytes, parent_tile)
      total_bytes = total_bytes + num_bytes
      ! To match initialization behavior, set any -999s to 0 in veg_type
      call mn_replace_low_values(st%veg_type_grid, -100.0, 0.0)


      call mn_static_read_tiled_hires_r4(new_tile_ioffset, new_tile_joffset, tile_nx, tile_ny, refine, pelist, surface_dir, "slope_type", "slope_type", st%slope_type_grid, num_bytes, parent_tile)
      total_bytes = total_bytes + num_bytes
      ! To match initialization behavior, set any -999s to 0 in slope_type
      call mn_replace_low_values(st%slope_type_grid, -100.0, 0.0)


      call mn_static_read_tiled_hires_r4(new_tile_ioffset, new_tile_joffset, tile_nx, tile_ny, refine, pelist, surface_dir, "maximum_snow_albedo", "maximum_snow_albedo", st%max_snow_alb_grid, num_bytes, parent_tile)
      total_bytes = total_bytes + num_bytes
      ! Set any -999s to 0.5
      call mn_replace_low_values(st%max_snow_alb_grid, -100.0, 0.5)


      ! Albedo fraction -- read and calculate
      call mn_static_read_tiled_hires_r4(new_tile_ioffset, new_tile_joffset, tile_nx, tile_ny, refine, pelist, surface_dir, "facsf", "facsf", st%facsf_grid, num_bytes, parent_tile)
      total_bytes = total_bytes + num_bytes
      ...
      do i=lbound(st%facsf_grid,1),ubound(st%facsf_grid,1)
        do j=lbound(st%facsf_grid,2),ubound(st%facsf_grid,2)
          if (st%facsf_grid(i,j) .lt. -100) then
            st%facsf_grid(i,j) = 0
            st%facwf_grid(i,j) = 0
          else
            st%facwf_grid(i,j) = 1.0 - st%facsf_grid(i,j)
          endif
        enddo
      enddo
      ...
      call mn_static_read_tiled_hires_r4(new_tile_ioffset, new_tile_joffset, tile_nx, tile_ny, refine, pelist, surface_dir, "snowfree_albedo", "visible_black_sky_albedo", st%alvsf_grid, num_bytes, parent_tile, time=month)
      total_bytes = total_bytes + num_bytes
      call mn_static_read_tiled_hires_r4(new_tile_ioffset, new_tile_joffset, tile_nx, tile_ny, refine, pelist, surface_dir, "snowfree_albedo", "visible_white_sky_albedo", st%alvwf_grid, num_bytes, parent_tile, time=month)
      total_bytes = total_bytes + num_bytes
      call mn_static_read_tiled_hires_r4(new_tile_ioffset, new_tile_joffset, tile_nx, tile_ny, refine, pelist, surface_dir, "snowfree_albedo", "near_IR_black_sky_albedo", st%alnsf_grid, num_bytes, parent_tile, time=month)
      total_bytes = total_bytes + num_bytes
      call mn_static_read_tiled_hires_r4(new_tile_ioffset, new_tile_joffset, tile_nx, tile_ny, refine, pelist, surface_dir, "snowfree_albedo", "near_IR_white_sky_albedo", st%alnwf_grid, num_bytes, parent_tile, time=month)
      total_bytes = total_bytes + num_bytes

      ! Set the -999s to small value of 0.06, matching initialization code in chgres

      call mn_replace_low_values(st%alvsf_grid, -100.0, 0.06)
      call mn_replace_low_values(st%alvwf_grid, -100.0, 0.06)
      call mn_replace_low_values(st%alnsf_grid, -100.0, 0.06)
      call mn_replace_low_values(st%alnwf_grid, -100.0, 0.06)
